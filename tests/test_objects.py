...
from src.dataset import load_train_dataset, transform_train_dataset, get_train_dataloader
from src.dataset import load_val_dataset, transform_val_dataset, get_val_dataloader
from src.dataset import load_test_dataset, transform_test_dataset, get_test_dataloader
...
def test_load_train_dataset(train_dataset):
    assert isinstance(train_dataset, ImageFolder), "Dataset should be an instance of ImageFolder"
...
def test_get_train_dataloader(train_dataset):
    ...
    assert isinstance(dataloader, DataLoader), "Dataloader should be an instance of DataLoader"
    for images, labels in dataloader:
        assert images.shape[0] == batch_size, "Batch size should match the expected value"
        break

def test_load_val_dataset(val_dataset):
    assert isinstance(val_dataset, ImageFolder), "Dataset should be an instance of ImageFolder"
...
def test_load_test_dataset(test_dataset):
    assert isinstance(test_dataset, ImageFolder), "Dataset should be an instance of ImageFolder"
